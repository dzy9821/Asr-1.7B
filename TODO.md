...
### 5. ASR HTTP 客户端需向客户端透传 vLLM 错误码 `[执行]`
...
### 11. README 移除 `asr_queue_depth` 指标，保留 `asr_error_rate` `[执行]`
...
### 12. URL 从 `/tuling/asr/v3` 改为 `/tuling/ast/v3` `[执行]`
...
### 13. 默认端口改为 8856，简化 README 启动命令 `[执行]`
