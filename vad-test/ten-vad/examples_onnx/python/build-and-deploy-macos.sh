...
if [[ -n "$ORT_ROOT" ]]; then
    cmake -S . -B build-macos -DORT_ROOT="$ORT_ROOT" -DPython_EXECUTABLE="$PYTHON_EXECUTABLE" -DPython_ROOT_DIR="$(dirname $(dirname $PYTHON_EXECUTABLE))"
else
    cmake -S . -B build-macos -DPython_EXECUTABLE="$PYTHON_EXECUTABLE" -DPython_ROOT_DIR="$(dirname $(dirname $PYTHON_EXECUTABLE))"
fi
