...
### 1. 根目录文件
*   **`main.py`**: 整个 FastAPI 服务的启动入口。负责注册各个 HTTP/WebSocket 路由（API 端点），并管理全局生命周期（在启动和关闭时初始化/销毁 ASR 服务与 ITN、VAD 进程池）。启动命令建议：`python -m uvicorn main:app --host 0.0.0.0 --port 8000 --ws-ping-interval 20 --ws-ping-timeout 300`。
...
### 2. 核心模块 (`src/core/`)
该模块提供项目的基础公共设施。
*   **`config.py`**: 极简的全局统一配置管理。所有参数（端口、并发上限、WebSocket 心跳控制、vLLM 地址、热词等）均通过 `os.getenv()` 读取环境变量，代码内提供默认值。支持的环境变量包括 `WS_PING_TIMEOUT`、`MP_QUEUE_LOG_INTERVAL_SEC`、`HOTWORDS` 等。
...
*   **`vad_pool.py` / `vad_service.py`**: 流式语音活动检测服务。采用多进程池（默认32实例）包装底层的 TenVAD，隔离 C/C++ 状态，并在服务启动时**主动预加载 VAD 模型实例（Eager Loading）**，彻底消除首个连接的冷启动延迟。通过哈希路由绑定连接与实例，内置**动态停顿阈值策略**，在识别准确度和延迟间取得平衡。
...
| **服务层** | `vad_pool.py`（VAD多进程池）、`asr_service.py`（异步及重试）、`itn_pool.py`（ITN多进程池） |
